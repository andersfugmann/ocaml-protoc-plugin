...
let merge: type t. t Spec.Deserialize.compound -> t -> t -> t = fun spec t t' -> match spec with
  | Spec.Deserialize.Basic (_field, Message _, _) -> failwith "Messages with defaults cannot happen"
  ...
  | Spec.Deserialize.Basic (_field, _spec, _) -> t'
    ...
    *)
  | Spec.Deserialize.Basic_req (_field, Message (module Message)) -> Message.merge t t'
  | Spec.Deserialize.Basic_req (_field, _spec) -> t'
  | Spec.Deserialize.Basic_opt (_field, Message (module Message)) ->
    begin
      match t, t' with
      | None, None -> None
      | Some t, None -> Some t
